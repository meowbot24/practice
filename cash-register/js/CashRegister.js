angular.module('cashRegisterApp.services', [
  'Item.services'])
  .factory('cashRegister', function(Item, ItemList) {
    ...
    var cashRegister = {
      ...
      add: function(itemName, quantity) {
        ...
        // looping through ItemList.list to match name to find price & add to total
        for (var item in ItemList.list) {
          if (itemName === ItemList.list[item].name) {
            this.lastItemCost = ItemList.list[item].price * quantity;
      ...
      voidItem: function(itemName, quantity) {
        for (var item in ItemList.list) {
          if (itemName === ItemList.list[item].name) {
            this.total -= ( ItemList.list[item].price * quantity );
            console.log(quantity + ' ' + ItemList.list[item].name + ' has been voided.');

            // remove from bill
            for (var billItem in this.bill) {
              if (ItemList.list[item].name === this.bill[billItem].name) {
